## ---------------------------
##
## Script name: Model_Structural
##
## Purpose of script: Run seasonal models of mosquito community abundance and presence data on only structural covariates
##
## Author: Daniel Smith
##
...
covariates <- seasonal_pa %>% map(select, width:suburban, -eastings, -northings, starts_with("cw"),
                                  -exposure, -cleared) %>% 
  # New selection here - structural
  map(select, width:management)
...
ifelse(!dir.exists(file.path("Models", "Structural")), dir.create(file.path("Models", "Structural")), FALSE)
...
for (i in seq_along(models)) {
  ...
  # Filename Saving
  filename = file.path("Models", "Structural", paste0(names(models)[[i]], ".RData"))
  # Save file
