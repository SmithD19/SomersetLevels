## ---------------------------
##
## Script name: Model_EllenbergOnly
##
## Purpose of script: Run seasonal models of mosquito community abundance and presence data on only EllenbergOnly covariates
##
## Author: Daniel Smith
##
...
covariates <- seasonal_pa %>% map(select, width:suburban, -eastings, -northings, starts_with("cw"),
                                  -exposure, -cleared) %>% 
  # New selection here - EllenbergOnly
  map(select, cw_light:cw_nitrogen)
...
ifelse(!dir.exists(file.path("Models", "EllenbergOnly")), dir.create(file.path("Models", "EllenbergOnly")), FALSE)
...
for (i in seq_along(models)) {
  ...
  # Filename Saving
  filename = file.path("Models", "EllenbergOnly", paste0(names(models)[[i]], ".RData"))
  # Save file
