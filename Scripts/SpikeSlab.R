## ---------------------------
##
## Script name: HMSC_Season_Model_Ellenbergs
##
## Purpose of script: Run seasonal models of mosquito community abundance and presence data
##
## Author: Daniel Smith
##
...
# Variable selection jointly for all species
XSelect.joint = list()
...
for (i in 1:3) {
  ...
  XSelect.joint[[i]] = list()
  
  for (k in 2:nc) {
    covGroup = k
    spGroup = rep(1, ns)
    q = rep(qq, max(spGroup))
    XSelect.joint[[i]][[k - 1]] = list(covGroup = covGroup,
                                       spGroup = spGroup,
                                       q = q)
  }
}
...
for (i in 1:3) {
  ...
  models[[i]] <- Hmsc(
    ...
    X = X[[i]],
    # What covariates to select?
    XSelect = XSelect.joint[[i]],
    # Distribution
    distr = "probit",
    # Study Design
...
ifelse(!dir.exists(file.path("Models", "SpikeSlab")), dir.create(file.path("Models", "SpikeSlab")), FALSE)
...
for (i in seq_along(models)) {
  ...
  # Filename Saving
  filename = file.path("Models", "SpikeSlab", paste0(names(models)[[i]], ".RData"))
  # Save file
