...
# This is a environmental only model
models[[2]] <- Hmsc(
  # A matrix of species occurrence/abundance records
  Y = as.matrix(Y),
  # A formula object for linear regression
...
# This is a spatiotemporal model only
models[[3]] <- Hmsc(
  # A matrix of species occurrence/abundance records
  Y = as.matrix(Y),
  # A formula object for linear regression
...
names(models) <- c("m_full", "m_env", "m_space")
...
filepath = file.path("Models", "Probit")
...
for (i in seq_along(models)) {
  ...
  save.image(paste0(filepath, "/Model", names(models)[[i]], ".RData"))
